    ...
    <script type="text/babel">
        ...
        const App = () => {
            ...
            const [records, setRecords] = useState([
                { id: '20260331-001', employeeId: '1024', applicant: '王大錘', type: '事前', category: '一般上班日', reimbursementType: '補休', startDateTime: '2026-03-31 18:00', endDateTime: '2026-03-31 20:00', totalHours: '2.0', reason: '專案上線準備', status: '待簽核', department: '行政管理部' },
                { id: '20260331-002', employeeId: '1025', applicant: '李華', type: '事後', category: '休息日', reimbursementType: '計薪', startDateTime: '2026-03-30 22:00', endDateTime: '2026-03-31 02:30', totalHours: '4.5', reason: '修復緊急故障', status: '已核准', department: '工程部' },
            ]);
                    ...
                    <div className="max-w-4xl mx-auto animate-in fade-in slide-in-from-bottom-4 duration-500 font-sans">
                        <div className="bg-white p-8 rounded-2xl shadow-sm border border-slate-200 text-left">
                            <div className="flex justify-between items-center mb-6">
                                ...
                                <div className="text-[11px] font-bold text-slate-400 bg-slate-50 px-3 py-1 rounded-lg border border-slate-100 flex items-center gap-1.5 uppercase tracking-wider">
                            ...
                            <form onSubmit={handleSubmit} className="space-y-6">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 p-5 bg-slate-50/50 rounded-2xl border border-slate-100 font-sans">
                                    <div className="space-y-2">
                                        <label className="text-xs font-bold text-slate-500 uppercase tracking-widest">員工編號 (4位數)</label>
                                    ...
                                    <div className="space-y-2">
                                        <label className="text-xs font-bold text-slate-500 uppercase tracking-widest">員工姓名</label>
                                ...
                                <div className="flex items-center justify-between p-4 bg-slate-900 rounded-xl text-white shadow-xl font-sans">
                                    ...
                                    <div className="text-right font-sans"><span className="text-3xl font-black font-mono text-blue-400 tracking-tighter">{calculatedHours}</span><span className="ml-1 text-sm font-bold text-slate-400 uppercase">小時</span></div>
                                </div>
                                <div className="space-y-2 text-left font-sans"><label className="text-sm font-bold text-slate-700">加班事由說明</label><textarea rows="3" required placeholder="請詳細描述加班原因、欲達成之目標或工作內容..." className="w-full p-4 bg-slate-50 border border-slate-100 rounded-xl focus:bg-white focus:ring-2 focus:ring-blue-500 transition-all font-sans" value={formData.reason} onChange={(e) => setFormData({...formData, reason: e.target.value})}></textarea></div>
                                <div className="pt-4 font-sans"><button type="submit" disabled={parseFloat(calculatedHours) <= 0} className={`w-full font-bold py-4 rounded-xl shadow-lg transition-all active:scale-[0.98] flex items-center justify-center gap-2 ${parseFloat(calculatedHours) > 0 ? 'bg-blue-600 hover:bg-blue-700 text-white shadow-blue-200' : 'bg-slate-200 text-slate-400 cursor-not-allowed'}`}><Icon name="check-circle" size={20} /> 提交加班申請案</button>
                                {parseFloat(calculatedHours) <= 0 && formData.startDate && <p className="text-xs text-red-500 mt-2 text-center font-bold font-sans italic">⚠️ 結束時間必須晚於開始時間。</p>}</div>
                            </form>
                            <div className="mt-12 p-6 bg-amber-50 border border-amber-200 rounded-2xl text-left font-sans shadow-sm">
                                ...
                                <ul className="space-y-3 text-sm text-amber-900/80 leading-relaxed font-sans">
                                    <li className="flex gap-2 font-bold"><span className="shrink-0 bg-amber-200 text-amber-800 w-5 h-5 rounded-full flex items-center justify-center text-[10px] font-black">A</span><span>加班申請須事前由直屬主管核准，始得進行加班。</span></li>
                                    <li className="flex gap-2 font-bold"><span className="shrink-0 bg-amber-200 text-amber-800 w-5 h-5 rounded-full flex items-center justify-center text-[10px] font-black">B</span><span>此單由各部門編序號並於加班後<span className="underline">七個工作日內</span>交至財務行政部辦理。</span></li>
                                    <li className="flex gap-2 font-bold"><span className="shrink-0 bg-amber-200 text-amber-800 w-5 h-5 rounded-full flex items-center justify-center text-[10px] font-black">C</span><span>此加班工時將依比率換算為補休時數或薪資。</span></li>
                                    <li className="flex gap-2 font-bold"><span className="shrink-0 bg-amber-200 text-amber-800 w-5 h-5 rounded-full flex items-center justify-center text-[10px] font-black">D</span><span>每月加班時數上限不得超過 46 小時。</span></li>
                                </ul>
                            </div>
                        </div>
                    </div>
