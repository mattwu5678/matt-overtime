...
const App = () => {
  ...
  const renderContent = () => {
    switch (activeTab) {
          ...
          <div className="max-w-4xl mx-auto animate-in fade-in slide-in-from-bottom-4 duration-500 pb-20">
            <div className="bg-white p-8 rounded-2xl shadow-sm border border-slate-200 text-left">
              <div className="flex justify-between items-center mb-8">
                ...
                <div className="text-xs font-bold text-slate-500 bg-slate-100 px-4 py-2 rounded-xl border border-slate-200 flex items-center gap-1.5 uppercase tracking-wider">
                    ...
                    <div className="flex flex-col md:flex-row gap-4">
                      <div className="w-full md:w-52 relative">
                        <input
                          ...
                          className="w-full p-3.5 bg-white border border-slate-200 rounded-xl text-base cursor-pointer focus:ring-2 focus:ring-blue-500 transition-all outline-none"
          ...
          <div className="max-w-4xl mx-auto space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500 text-left">
            ...
            {records.filter(r => r.status === '待簽核').length === 0 ? (
              ...
              records.filter(r => r.status === '待簽核').map(record => (
                <div key={record.id} className="bg-white p-8 rounded-2xl shadow-sm border border-slate-200 flex flex-col xl:flex-row justify-between items-start xl:items-center gap-8">
                  <div className="flex-1 space-y-4 w-full">
                    <div className="flex items-center flex-wrap gap-3">
                      <span className="px-3 py-1 bg-slate-100 text-slate-600 rounded-lg text-xs font-bold font-mono tracking-wider">單號: {record.id}</span>
                      ...
                      <h3 className="font-bold text-xl text-slate-800 ml-2">{record.applicant}</h3>
                    </div>
                    <div className="flex flex-col sm:flex-row sm:items-center gap-3 sm:gap-6 text-sm font-mono text-slate-500 font-bold">
                      <div className="bg-slate-50 px-4 py-2.5 rounded-xl border border-slate-100">開始：{record.startDateTime}</div>
                      <ArrowRight size={18} className="text-slate-300 hidden sm:block" />
                      <div className="bg-slate-50 px-4 py-2.5 rounded-xl border border-slate-100">結束：{record.endDateTime}</div>
                    </div>
                    <div className="bg-slate-50 p-4 rounded-xl text-base text-slate-600 border-l-4 border-slate-300 italic">事由：{record.reason}</div>
                  </div>
                  <div className="flex gap-3 w-full xl:w-auto shrink-0">
                    <button onClick={() => handleApprove(record.id, '已核准')} className="flex-1 xl:flex-none flex items-center justify-center gap-2 bg-green-600 hover:bg-green-700 text-white px-8 py-3.5 rounded-2xl font-bold transition-all shadow-md text-base"><Check size={20} /> 核准</button>
                    <button onClick={() => handleApprove(record.id, '已駁回')} className="flex-1 xl:flex-none flex items-center justify-center gap-2 bg-white hover:bg-red-50 text-red-500 border border-red-100 px-8 py-3.5 rounded-2xl font-bold transition-all text-base"><X size={20} /> 駁回</button>
                  </div>
                </div>
              ))
            )}
      ...
      case 'query':
        return (
          <div className="max-w-6xl mx-auto bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden animate-in fade-in duration-500 text-left">
            <div className="p-8 border-b border-slate-100 flex flex-wrap items-center justify-between gap-6 bg-slate-50/50">
              <h2 className="font-bold text-xl text-slate-800">加班紀錄查詢與彙整</h2>
              <div className="relative">
                <Search className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400" size={20} />
                <input type="text" placeholder="搜尋姓名、工號..." className="pl-12 pr-5 py-3 bg-white border border-slate-200 rounded-xl text-base focus:ring-2 focus:ring-blue-500 w-80 shadow-sm" />
              </div>
            </div>
              ...
              <table className="w-full border-collapse">
                <thead>
                  <tr className="text-slate-400 text-xs uppercase font-black border-b border-slate-100 bg-slate-50/30">
                    <th className="px-8 py-5 text-left">單號</th>
                    <th className="px-8 py-5 text-left">姓名 / 工號</th>
                    <th className="px-8 py-5 text-left">加班時段</th>
                    <th className="px-8 py-5 text-left">時數 / 補償</th>
                    <th className="px-8 py-5 text-center">狀態</th>
                ...
                <tbody className="divide-y divide-slate-50">
                  {records.map(record => (
                    <tr key={record.id} className="hover:bg-slate-50/80 transition-colors group">
                      <td className="px-8 py-5 font-mono text-xs font-bold text-slate-500">{record.id}</td>
                      <td className="px-8 py-5">
                        <div className="font-bold text-slate-800 text-base">{record.applicant}</div>
                        <div className="text-xs text-slate-400 font-mono tracking-wider mt-0.5">#{record.employeeId}</div>
                      </td>
                      <td className="px-8 py-5 font-mono text-xs leading-relaxed">
                        <div className="text-slate-700 font-bold">始：{record.startDateTime}</div>
                        <div className="text-slate-400">迄：{record.endDateTime}</div>
                      </td>
                      <td className="px-8 py-5">
                        <div className="flex items-center gap-2">
                          <span className="text-blue-600 font-mono font-black text-base">{record.totalHours}H</span>
                          <span className={`text-[11px] px-2 py-0.5 rounded-md font-bold border ${record.reimbursementType === '補休' ? 'bg-indigo-50 text-indigo-600 border-indigo-100' : 'bg-emerald-50 text-emerald-600 border-emerald-100'}`}>{record.reimbursementType}</span>
                        </div>
                      </td>
                      <td className="px-8 py-5 text-center">
                        <span className={`inline-flex items-center px-4 py-1.5 rounded-full text-xs font-bold shadow-sm ${record.status === '已核准' ? 'bg-green-50 text-green-600' : record.status === '待簽核' ? 'bg-orange-50 text-orange-600' : 'bg-red-50 text-red-600'}`}>{record.status}</span>
                      </td>
                      <td className="px-8 py-5 text-right">
                        <button className="text-slate-300 group-hover:text-blue-600 transition-colors"><ChevronRight size={24} /></button>
                      </td>
                    </tr>
                  ))}
                </tbody>
      ...
      case 'settings':
        return (
          <div className="max-w-4xl mx-auto grid grid-cols-1 lg:grid-cols-3 gap-8 animate-in fade-in duration-500 text-left">
            <div className="lg:col-span-1 text-center">
              <div className="bg-white p-10 rounded-2xl shadow-sm border border-slate-200">
                <div className="w-20 h-20 bg-blue-100 rounded-full flex items-center justify-center text-blue-600 mx-auto mb-6 border-2 border-blue-200 font-black text-xl shadow-inner">Admin</div>
                <h3 className="font-bold text-xl text-slate-800">王大錘 經理</h3>
                <p className="text-slate-400 text-xs mt-1 mb-8 font-mono font-bold uppercase tracking-widest">工號: 1024</p>
                <button 
                  onClick={() => setIsSupervisor(!isSupervisor)}
                  className={`w-full py-3.5 rounded-2xl font-bold transition-all border-2 text-base shadow-sm ${isSupervisor ? 'bg-slate-900 border-slate-900 text-white' : 'bg-white border-slate-200 text-slate-600 hover:bg-slate-50'}`}
                >
                  {isSupervisor ? '身份：管理員' : '身份：一般員工'}
                </button>
              </div>
            </div>
            <div className="lg:col-span-2 space-y-6">
              <div className="bg-white p-8 rounded-2xl shadow-sm border border-slate-200">
                <h3 className="font-bold text-base text-slate-800 mb-6 flex items-center gap-2"><Settings size={20} /> 系統參數設定</h3>
                <div className="space-y-4">
                  <div className="flex justify-between items-center p-5 bg-slate-50 rounded-2xl border border-slate-100">
                    <div className="text-sm font-bold text-slate-700">自動生成單據流水號</div>
                    <span className="text-xs font-bold text-blue-600 bg-white border border-blue-100 px-3 py-1 rounded-lg font-mono">{currentSerialNumber}</span>
                  </div>
                  <div className="flex justify-between items-center p-5 bg-slate-50 rounded-2xl border border-slate-100">
                    <div className="text-sm font-bold text-slate-700">預設登入者自動帶入</div>
                    <span className="text-xs font-bold text-green-600 bg-white border border-green-100 px-3 py-1 rounded-lg">啟用中</span>
                  </div>
                </div>
              </div>
            </div>
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex overflow-hidden font-sans antialiased text-slate-900">
      <aside className={`bg-slate-900 text-white flex flex-col transition-all duration-300 z-50 ${isSidebarOpen ? 'w-72' : 'w-24'}`}>
        <div className="h-20 flex items-center justify-between px-6 border-b border-slate-800">
          <div className="flex items-center gap-4 overflow-hidden">
            <div className="bg-blue-600 p-2.5 rounded-xl shrink-0 shadow-lg">
              <FileText size={24} className="text-white" />
            </div>
            {isSidebarOpen && (
              <div className="whitespace-nowrap animate-in fade-in slide-in-from-left-2 duration-300">
                <h1 className="text-base font-black tracking-wider text-slate-100 uppercase">內部管理系統</h1>
              </div>
            )}
          </div>
          <button onClick={() => setIsSidebarOpen(!isSidebarOpen)} className="p-2 rounded-xl hover:bg-slate-800 transition-colors text-slate-400 hover:text-white">
            <Menu size={20} />
          </button>
        </div>
             ...
             <div className="p-5 bg-slate-800/50 rounded-2xl border border-slate-700/50 shadow-inner">
               ...
               <div className="space-y-4">
                 <div className="flex items-center justify-between text-xs font-bold">
                   ...
                   <span className="text-blue-400 font-bold">王大錘</span>
                 </div>
                 <div className="flex items-center justify-between text-xs font-bold">
                   <span className="text-slate-500">權限級別</span>
                   <span className={isSupervisor ? "text-green-400" : "text-slate-400"}>{isSupervisor ? "管理人員" : "一般權限"}</span>
                 </div>
               </div>
             </div>
           )}
        ...
        <div className="p-4 border-t border-slate-800">
          <div className={`flex items-center p-4 bg-slate-800/50 rounded-2xl ${!isSidebarOpen && 'justify-center'} shadow-md`}>
            ...
            {isSidebarOpen && (
              <div className="ml-4 overflow-hidden text-left animate-in fade-in duration-300">
                <p className="text-[10px] font-bold text-slate-500 font-mono tracking-widest uppercase">EMP ID #1024</p>
              </div>
            )}
          </div>
        </div>
      ...
      <main className="flex-1 flex flex-col overflow-hidden">
        <header className="h-20 border-b border-slate-200 bg-white/80 backdrop-blur-md z-40 px-10 flex items-center justify-between shadow-sm">
          ...
          <div className="flex items-center gap-6">
            <div className="text-xs text-slate-400 font-mono font-black tracking-widest bg-slate-50 px-4 py-2 rounded-lg border border-slate-100">{new Date().toLocaleDateString('zh-TW', { year: 'numeric', month: '2-digit', day: '2-digit' }).replace(/\//g, '-')}</div>
            <button className="w-10 h-10 rounded-xl bg-slate-50 flex items-center justify-center text-slate-400 hover:bg-red-50 hover:text-red-500 transition-all border border-slate-100">
              <LogOut size={20} />
            </button>
          </div>
        </header>
...
const TabItem = ({ active, label, onClick, badge }) => (
  <button
    onClick={onClick}
    className={`h-full px-8 flex items-center gap-3 transition-all relative font-bold text-sm tracking-wide ${
      active ? 'text-blue-600' : 'text-slate-400 hover:text-slate-800'
    }`}
  >
    {label}
    {badge > 0 && (
      <span className="px-2 py-0.5 bg-red-500 text-white text-[10px] rounded-full min-w-[20px] text-center font-black animate-pulse shadow-sm">
        {badge}
      </span>
    )}
    {active && (
