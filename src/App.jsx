...
const App = () => {
  ...
  const handleEmployeeIdChange = (id) => {
    ...
    setFormData(prev => ({
      ...prev,
      employeeId: id,
      employeeName: found ? found.name : prev.employeeName // 若找到則更新，沒找到保持現狀供手動修改
    }));
  };
  ...
  const handleEmployeeNameChange = (name) => {
    ...
    setFormData(prev => ({
      ...prev,
      employeeName: name,
      employeeId: found ? found.id : prev.employeeId // 若找到則更新
    }));
  };
  ...
  const renderContent = () => {
    switch (activeTab) {
                  ...
                  <div className="p-4 bg-slate-50 rounded-2xl border border-slate-100">
                    ...
                    <div className="flex flex-col md:flex-row gap-4 font-sans">
                      ...
                      <div className="flex flex-1 gap-2">
                        <select className="flex-1 p-3 bg-white border border-slate-200 rounded-xl text-sm font-mono focus:ring-2 focus:ring-blue-500" value={formData.startHour} onChange={(e) => setFormData({...formData, startHour: e.target.value})}>
                          {hourOptions.map(h => <option key={h} value={h}>{h} 時</option>)}
                        </select>
                        <select className="flex-1 p-3 bg-white border border-slate-200 rounded-xl text-sm font-mono focus:ring-2 focus:ring-blue-500" value={formData.startMinute} onChange={(e) => setFormData({...formData, startMinute: e.target.value})}>
                  ...
                  <div className="p-4 bg-slate-50 rounded-2xl border border-slate-100 font-sans">
                    ...
                    <div className="flex flex-col md:flex-row gap-4 font-sans">
                      ...
                      <div className="flex flex-1 gap-2 font-sans">
                        <select className="flex-1 p-3 bg-white border border-slate-200 rounded-xl text-sm font-mono focus:ring-2 focus:ring-blue-500" value={formData.endHour} onChange={(e) => setFormData({...formData, endHour: e.target.value})}>
                          {hourOptions.map(h => <option key={h} value={h}>{h} 時</option>)}
                        </select>
                        <select className="flex-1 p-3 bg-white border border-slate-200 rounded-xl text-sm font-mono focus:ring-2 focus:ring-blue-500" value={formData.endMinute} onChange={(e) => setFormData({...formData, endMinute: e.target.value})}>
                          <option value="00">00 分</option>
                          <option value="30">30 分</option>
                        </select>
                      </div>
      ...
      case 'approve':
        return (
          <div className="max-w-4xl mx-auto space-y-4 animate-in fade-in slide-in-from-bottom-4 duration-500 text-left font-sans font-sans">
          ...
          <div className="max-w-4xl mx-auto grid grid-cols-1 lg:grid-cols-3 gap-8 animate-in fade-in duration-500 text-left font-sans">
            <div className="lg:col-span-1 text-center font-sans">
              <div className="bg-white p-8 rounded-2xl shadow-sm border border-slate-200">
                <div className="w-16 h-16 bg-blue-100 rounded-full flex items-center justify-center text-blue-600 mx-auto mb-4 border border-blue-200 font-black uppercase">Admin</div>
              ...
              <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
                ...
                <div className="space-y-4">
                  <div className="flex justify-between items-center p-3 bg-slate-50 rounded-xl border border-slate-100">
                    ...
                    <span className="text-[10px] font-bold text-blue-600 bg-white border border-blue-100 px-2 py-0.5 rounded-lg">{currentSerialNumber}</span>
                  </div>
                  <div className="flex justify-between items-center p-3 bg-slate-50 rounded-xl border border-slate-100">
      ...
      <aside className={`bg-slate-900 text-white flex flex-col transition-all duration-300 z-50 ${isSidebarOpen ? 'w-64' : 'w-20'}`}>
        ...
        <div className="p-3 border-t border-slate-800 font-sans">
          <div className={`flex items-center p-3 bg-slate-800/50 rounded-2xl ${!isSidebarOpen && 'justify-center'}`}>
            ...
            {isSidebarOpen && (
              <div className="ml-3 overflow-hidden text-left font-sans animate-in fade-in duration-300">
                <p className="text-[11px] font-bold text-slate-500 font-mono tracking-tighter uppercase font-sans">ID #1024</p>
              </div>
            )}
          </div>
        </div>
...
const TabItem = ({ active, label, onClick, badge }) => (
  <button
    onClick={onClick}
    className={`h-full px-6 flex items-center gap-2 transition-all relative font-black text-[13px] font-sans ${
